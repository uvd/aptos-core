    ...
    > Bootstrapper<StorageSyncer, StreamingClient>
{
    ...
    async fn initialize_active_data_stream(
        ...
        // If we've synced to the highest known, there's nothing left to do
        ...
        match self.driver_configuration.config.bootstrapping_mode {
            BootstrappingMode::DownloadLatestStates => {
                // Check if it's more efficient to sync all transactions (instead of a new snapshot)
                if highest_synced_version != GENESIS_TRANSACTION_VERSION {
                            ...
                            .num_versions_to_skip_snapshot_sync
                    {
                        ...
                        You are currently {:?} versions behind the latest snapshot version ({:?}). Either: (i) select a \
                        different syncing mode; or (ii) delete your storage and restart your node.", num_versions_behind, highest_known_ledger_version);
                    }
                }
            ...
            let last_persisted_state_index = self
                ...
                .map_err(|error| {
                    Error::StorageError(format!(
                        "Failed to get the last persisted state value index from metadata storage: {:?}",
                        error
                    ))
                })? as u64;
